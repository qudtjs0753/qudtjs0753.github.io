...
## 스택 카나리 심화 — 구조와 우회 가능성

### 카나리 값의 내부 구조 (NULL 바이트)

스택 카나리는 단순한 무작위 값이 아니라 특별한 구조를 갖고 있다.

**64비트 시스템:**
- 카나리는 8바이트이지만, 첫 1바이트가 항상 `\x00`으로 고정
- 실질적으로 랜덤한 부분은 7바이트에 불과함
- 따라서 브루트포스 난이도: 256^7 ≈ 72조 번

**32비트 시스템:**
- 4바이트 중 첫 1바이트가 `\x00`
- 실질적으로 3바이트만 랜덤
- 브루트포스 난이도: 256^3 = 16백만 번

**NULL 바이트가 있는 이유:**

```
[Canary Value] = 0x00 XX XX XX XX XX XX XX  (64bit)
                 ↑
            첫 1바이트 고정
```

- `strlen()`, `strcpy()` 같은 문자열 함수는 `\x00`을 만나면 읽기를 멈춤
- 따라서 카나리 시작 부분이 `\x00`이면, 문자열 기반 오버플로우가 카나리까지 도달하지 못함
- 버퍼(배열)와 카나리 사이의 "**완충장치**" 역할

### 어셈블리 수준 동작 원리

스택 카나리는 **TLS(Thread Local Storage)**라는 메커니즘을 사용한다.

**함수 시작:**
```asm
push rbp
mov rbp, rsp
mov rax, qword ptr fs:[0x28]      ; TLS에서 카나리 로드
mov qword ptr [rbp - 8], rax      ; 스택에 카나리 복사
```

**함수 종료:**
```asm
mov rax, qword ptr [rbp - 8]      ; 스택의 카나리 로드
xor rax, qword ptr fs:[0x28]      ; TLS의 원본과 XOR 비교
jne  __stack_chk_fail             ; 같으면 ZF=1, 다르면 ZF=0 → __stack_chk_fail 호출
```

**동작 흐름:**
1. `fs:0x28` = TLS에 저장된 원본 카나리
2. 함수 시작 시 이 값을 스택에 복사
3. 함수 종료 시 스택의 값과 TLS의 값을 비교 (XOR = 0이면 같음)
4. 다르면 `__stack_chk_fail` 호출 → `"stack smashing detected"` 출력 후 종료

### 카나리 우회 가능성

카나리는 완벽한 방어 메커니즘은 아니다.

**이론적 우회 조건:**

카나리는 **값 비교만** 수행하므로, 공격자가 다음 중 하나를 만족하면 우회 가능하다:

1. **카나리 값을 읽어낼 수 있는 취약점 존재**
   - Information leak (메모리 읽기 취약점)
   - 예: format string 취약점으로 스택 내용 노출
   - 예: 초기화되지 않은 메모리 사용

2. **TLS의 `fs:0x28` 직접 접근**
   - 사용자 레벨 코드에서 불가능 (CPU 제어 명령어 필요)
   - 커널 취약점 필요

3. **카나리 전까지만 오버플로우**
   - RBP나 return address를 건드리지 않고, 다른 중요한 변수만 변조
   - 하지만 이 경우 공격의 효과가 제한적

**실제 공격 시나리오:**

```cpp
// 이 프로그램에 format string 취약점이 있다면:
printf(user_input);  // ← format string 취약점

// 공격자는 "%p %p %p ..." 로 스택 메모리 읽기 가능
// → 카나리 값을 얻을 수 있음
// → 올바른 카나리로 return address 변조 가능
```

**결론:**

카나리는 **독립적인 취약점** 없이는 우회 불가능하다.
즉, 버퍼 오버플로우 취약점 **하나만**으로는 공격 불가능하지만,
버퍼 오버플로우 + 정보 누출(Information Leak) 같은 **두 개 이상의 취약점**이 결합되면 우회 가능하다.
...
이를 통해 학습한 스택 메모리 구조와 보안 메커니즘을 정리한 것이다.
...
- [쉽게 이해하는 스택 카나리 - kn1ght](https://blog.naver.com/luexr/223155451155)
  — 어셈블리 수준에서 카나리 동작, TLS/FS 레지스터, 우회 가능성을 실습과 함께 설명

- [GCC Stack Protector Options](https://gcc.gnu.org/onlinedocs/gcc/Instrumentation-Options.html)
  — `-fstack-protector` 관련 공식 문서

- [OWASP: Buffer Overflow](https://owasp.org/www-community/vulnerabilities/Buffer_Overflow)
  — 버퍼 오버플로우 공격 유형과 방어 방법 개요
