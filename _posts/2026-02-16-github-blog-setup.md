...
| **autoconf** | 시스템마다 다른 환경 차이를 파악하고 빌드 설정 스크립트를 자동 생성 | Ruby의 `./configure` 단계에서 |
...
```ruby
gem "jekyll-theme-chirpy", "~> 7.4", ">= 7.4.1"   # Chirpy 테마
gem "html-proofer", "~> 5.0", group: :test          # HTML 검증 도구 (테스트용)
```

**Gemfile.lock** — "실제로 설치된 정확한 버전 기록"
- `bundle install` 실행 시 자동 생성됨
- 이 프로젝트에서는 `.gitignore`에 포함되어 git에 커밋되지 않는다
- GitHub Actions에서 빌드 시 자체적으로 `Gemfile.lock`을 생성하므로 문제없음
