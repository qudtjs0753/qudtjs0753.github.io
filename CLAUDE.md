...
글 작성 완료 후, 커밋 전에 다음 절차로 다관점 리뷰를 수행한다:

1. **관점 조사**: 글의 주제에 대해 위키피디아, 관련 논문, 기술 문서 등을 조사하여 해당 주제를 바라보는 다양한 관점(분야, 역할, 입장)을 파악한다
2. **페르소나 생성**: 조사 결과를 바탕으로, 글을 검토하기에 적합한 페르소나들을 동적으로 구성한다 (고정된 목록이 아닌, 주제에 따라 달라짐)
3. **페르소나별 리뷰**: 각 페르소나의 관점에서 글을 읽고 구체적인 피드백을 제시한다
4. **저자 판단**: 피드백을 검토하고 수용 여부를 결정한다

각 단계를 수행할 때 아래 형식으로 진행 상황을 출력한다:

```
## 다관점 페르소나 리뷰
- [x] 관점 조사 완료: (조사한 소스 요약)
- [x] 페르소나 생성: (생성된 페르소나 목록)
- [ ] 페르소나별 리뷰 진행 중...
- [ ] 저자 판단 대기
```
...
각 항목을 점검할 때 아래 형식으로 진행 상황을 출력한다:
